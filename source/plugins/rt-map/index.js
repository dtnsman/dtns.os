...
const AMAP_MAPS_API_KEY = require('../../setting/rtmap_setting.json').map_key

if(typeof window == 'undefined') globalThis.window = globalThis
window.rtmap_c = {}
// const rtmap_c_token_name = OBJ_TOKEN_NAME
// const rtmap_c_api_base   = OBJ_API_BASE
// const rtmap_c_token_root = OBJ_TOKEN_ROOT
const http_req = require('../../libs/http_request')
async function http_get_url(url) 
{
    let res = await http_req.http_get(url)
    return {json:()=>{
        return res
    }}
}

rtmap_c.routers =async function(app)
{
    if(!app) return 
    // if(!app.setChatC) return 
    // const urlParser = null
    app.all('/rtmap/tool/call',urlParser,session_filter,rtmap_c.callMapTool)//
    app.all('/rtmap/tool/list',urlParser,session_filter,rtmap_c.listTool)
}
rtmap_c.callMapTool = async function(req,res)
{
    let params = str_filter.get_req_data(req)
    if(!params.tool_name) return res.json({ret:false,msg:'param-tool_name is empty'})
    let ret = await rtmap_c.callTool({params:{name:params.tool_name,arguments:params}})
    console.info('callMapTool-ret:',ret )
    // console.info('send-mail-ret:',ret ,params)
    let isError= ret.isError
    try{
        let retJson= ret//ret.isError ? ret.content[0].text:JSON.parse(ret.content[0].text).return 
        res.json({ret:isError?false:true,msg:isError ? 'query map failed':'success',data:retJson})
    }catch(ex)
    {
        console.error('rtmap_c.callMapTool-parse-json:ex:'+ex,ex,ret)
        res.json({ret:false,msg:'parse data failed'})
    }
}

rtmap_c.listTool = function(req,res){
    ...
    let tools_md = '#地图工具\n'
    for(let i=0;i<MAPS_TOOLS.length;i++)
    {
        tools_md+='\n##tool_name：'+MAPS_TOOLS[i].name+'\n工具描述：'+(MAPS_TOOLS[i].description)+'\n参数描述：'+JSON.stringify(MAPS_TOOLS[i].inputSchema)+'\n'
    }
    res.json({ret:true,msg:'success',tools:MAPS_TOOLS,tools_md})
}
...
rtmap_c.callTool =  async function (request)
{
    try {
...
async function handleReGeocode(location) {
    ...
    const response = await http_get_url(url.toString());
...
// async function test() 
// {
//     let a= await handleGeo('禅城区祖庙','佛山市')
//     console.log('a:',a)

//     let res = await rtmap_c.callTool({params:{call_tool_name:'maps_geo',arguments:{address:'禅城区祖庙',city:'佛山市'}}})
//     console.log('res:',res)

//     rtmap_c.callMapTool({params:{call_tool_name:'maps_geo',address:'禅城区祖庙',city:'佛山市'}},{json:function(data){
