...
class WsApi extends EventEmitter {
  ...
  constructor(apiKey, apiSecret, passphrase,start=true) {
    super();

    if(!start) return 
    const processMsg = message => {
      console.log('processMsg-msg:',message)
    ...
    this._public.on('open', () => {
      console.log('_public is opened')
      // this._public.send(JSON.stringify({
      //     "op": "subscribe",
      //     "args": [{
      //         "channel": "mark-price",
      //         "instId": "XCH-USDT"
    ...
    if (apiSecret) {
      ...
      this._private = new WS('wss://wsaws.okx.com:8443/ws/v5/private');//business---可以订阅充值信息频道deposit-info
