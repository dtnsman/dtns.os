...
// 短信应用SDK AppID
var appid = 1400043953;  // SDK AppID是1400开头
...
// 短信模板ID，需要在短信应用中申请
var templateId = 0;  // NOTE: 这里的模板ID`7839`只是一个示例，真实的模板ID需要在短信控制台中申请
...
module.exports.send_sms = async function (nation_code,phone,code,template_id) {

    template_id = !template_id ? 45580 :template_id
    let isOkFlag = false;
    await new Promise((resolve,reject)=>{
        nation_code = !nation_code ? 86:nation_code;

        var ssender = qcloudsms.SmsSingleSender();
        var params = [code];//数组具体的元素个数和模板中变量个数必须一致，例如事例中templateId:5678对应一个变量，参数数组中元素个数也必须是一个


        var callback = function(err, res0, resData) {
            if (err) {
                // console.log("err: ", err);
                reject(err);
            } else {
                //console.log("request data: ", res0.req);
                //console.log("response data: ", resData);
                resolve(true);
            }
        }
        ssender.sendWithParam(nation_code,phone, template_id,
 ...
 * @returns {Promise<boolean>}
 */
module.exports.send_email = async function (email,subject,content) {

    var tocontent=content
    var param = {subject:subject,
        from_mail: "*",to:email,templateInvokeName:"*",html:tocontent,oc_key:"*!",is_normal:"1",ismuli:"no",useAddressList:0};

    var ret = await http_request.http_post("*",param);
    console.log("=========发送邮件========"+JSON.stringify(ret));

    return ret && ret.result ?true:false
}
...
module.exports.nation_code_list = nation_code_list
module.exports.check_nation_code =check_nation_code
